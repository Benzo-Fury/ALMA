...
// respond to user
const msg = await submitted.editReply({ embeds: [embed], components: [addButtonsEnabled()] });
...
collector.on("collect", async (ie: ButtonInteraction) => {
  if (ie.user.id !== i.user.id) {
    ...
    await ie.reply({
      content:
        "This is to be added. If you would like to have full conversations with an AI chatbot you can set up AI monitoring with /aichathub",
    });
    await submitted.editReply({ embeds: [embed], components: [addButtonsDisabled()] });
